        ...
        public async Task Do1(object? state)
        {
            var ohtTasks = Enumerable.Range(1, 3).Select(n => $"OHT_00{n}").Select(ohtId =>
            {
                ...
                var tasks = sensorIds.Select(async (sensorId, idx) =>
                {
                    ...
                    //var content = JsonConvert.SerializeObject(equip1_elementCollection);
                    //var putStringContent = GetContent(content);
                    //var putUrl = $@"http://localhost:5000/shells/{aasId}/aas/submodels/{submodelId}/submodel/submodelElements/{submodelElementId}";
                    //await httpClient.PutAsync(putUrl, putStringContent);

                    ////event fire
                    //var eventId = $"Sensor{idx}Event";
                    //var eventSubmodelId = "InputEvent";
                    //var eventPostUrl = $@"http://localhost:5000/shells/{aasId}/aas/submodels/{eventSubmodelId}/submodel/events/{eventId}";
                    //var eventStringContent = GetContent(content);
                    //await httpClient.PostAsync(eventPostUrl, eventStringContent);
                });
